var address,role,menu,fs,user,customer,p400,booking,immediate,finance_d,mysql,erp;
...
exports.index = function (req, res) {
    ...
    mysql.query(sql1 ,function(error1,obj1){
        ...
        mysql.query(sql2 ,function(error,obj){
            ...
            res.render('front/index',{address_s:obj,obj1:obj1});
        });
    });
};
...
exports.option1 = function (req, res) {
    if(!req.session.cuser){
        ...
        mysql.query(Sql ,function(error,obj){
            if(error){console.log(error);return false;}
            res.render('front/option1',{obj:obj});
        });
    }
};
    ...
    if(!req.session.cuser){
        ...
        mysql.query(Sql ,function(error,obj){
            ...
            mysql.query(sql5,function (err1, rows5) {
                ...
                res.render('front/option2',{record:obj,page:page,total:total,totalpage:totalpage,isFirstPage:isFirstPage,isLastPage:isLastPage});
            });
        });
    }
};
...
exports.w_login = function (req, res) {
    res.render('weixin/login',{openid:req.session.openid});
}

exports.w_reg = function (req, res) {
    res.render('weixin/reg',{openid:req.session.openid});
}

exports.booking_w_login = function (req, res) {
    res.render('weixin/bookinglogin',{openid:req.session.openid});
}

exports.booking_w_reg = function (req, res) {
    res.render('weixin/bookingreg',{openid:req.session.openid});
}
                ...
                if(!err && response.statusCode == 200){
                    ...
                    mysql.query(sql1 ,function(error,user){
                        ...
                        if(user[0]){
                            ...
                            page = (page && page > 0) ? page : 1;
                            var limit = (limit && limit > 0) ? limit : LIMIT;
                            var Sql = "select * from booking where  linktel = '"+ctel+"'  order by bookingtime desc";
                            var sql5 = "select count(*) as count from booking where linktel = '"+ctel+"'";
                            mysql.query(Sql ,function(error,obj){
                                if(error){console.log(error);return false;}
                                for(var i in obj){
                                    var myDate = obj[i].bookingtime;
                                ...
                                mysql.query(sql5,function (err1, rows5) {
                                    ...
                                    var isFirstPage = page == 1 ;
                                    var isLastPage = ((page -1) * limit + obj.length) == total;
                                    res.render('weixin/option',{record:obj,page:page,total:total,totalpage:totalpage,isFirstPage:isFirstPage,isLastPage:isLastPage});
                                });
                            });
                        }else{
                            //没有绑定
...
exports.booking_front = function (req, res) {
    ...
    mysql.query(sql0 ,function(err0,rows0){
      if(err0){console.log(err0);return false;}
    mysql.query(sql1 ,function(err1,rows1){
      if(err1){console.log(err1);return false;}
    mysql.query(sql4 ,function(err4,rows4){
      if(err4){console.log(err4);return false;}
      mysql.query(sql2 ,function(err2,rows2){
        ...
        mysql.query(sql3 ,function(err3,rows3){
          ...
          res.render('front/booking',{url:req.url,user_s:rows1,address_s:rows2,menu_s:rows3,favorable:favorable,canbooking:rows0[0].canbooking,la:rows4[0]});
        });  
      });
    });
    });
...
exports.getAddressByAPI = function(req,res){
    ...
    console.log(_url);
    request({url:_url,timeout: 3000},function(err,response,body){
        if(!err && response.statusCode == 200){
            res.send(body);
        }
    });     
                                        ...
                                        mysql.query(sql4 ,function(err4,rows4){
                                            if(err4){console.log(err4);return false;}
                                                mysql.query(sql2 ,function(err2,rows2){
                                                    ...
                                                    mysql.query(sql3 ,function(err3,rows3){
                                                        if(err3){console.log(err3);return false;}
                                                            res.render('weixin/booking',{url:req.url,user_s:rows1,address_s:rows2,menu_s:rows3,favorable:favorable,canbooking:rows0[0].canbooking,la:rows4[0]});
                                                    });  
                                            });
                                    });
                                });
...
exports.w_list = function (req,res) {
    ...
    mysql.query(sql3 ,function(err3,rows3){
        if(err3){console.log(err3);return false;}
        res.render('weixin/list',{url:req.url,menu_s:rows3});
    });         
}
...
exports.uploadsuccess = function (req, res) {
    var p = req.query.p;
    res.render('cms/uploadsuccess',{url:getUrl(req),p:p});
};
...
exports.booking = function (req, res) {
    res.render('cms/booking',{url:getUrl(req)});
};

exports.immediate = function (req, res) {
    res.render('cms/immediate',{url:getUrl(req)});
};
...
exports.s400 = function (req, res) {
    res.render('cms/400success',{url:"/cms/400"});
};
...
exports.customer = function (req, res) {
    res.render('cms/customer',{url:getUrl(req)});
};
    ...
    request({url:url,method:'POST',body: formData},function(err,response,body){
        if(!err && response.statusCode == 200){
            ...
            if(!req.session.cuser){
                ...
                res.render('pay',{prepay_id:tmp1[0],code_url:tmp3[0],total_fee:total_fee/100});
            }
            //res.redirect(tmp3[0]);
        }
    });
...
exports.erp_material = function (req, res) {
	...
	mysql.query(sql1 ,function(error,obj){
        ...
        mysql.query(sql2 ,function(error,obj2){
	        if(error){console.log(error);return false;}
	        res.render('erp/material',{obj:obj,obj2:obj2});
	    });
    });
};
