...
function sql_insert(req, res) {
    ...
    var insertSql = "insert into ordergh (date,numA,numB,numC,numD,numE,priceTotal) values ('"+date+"','"+numA+"','"+numB+"','"+numC+"','"+numD+"','"+numE+"','"+priceTotal+"')";
...
function sql_delete(req, res) {
    var id = req.param('id');
    var deleteSql = "delete from ordergh where id = "+id;
...
function sql_select(req, res) {
    var id = req.param('id');
    var delSql = "select * from ordergh where id = "+id;
...
function sql_update(req, res) {
    ...
    var updateSql = "update ordergh set date = '"+date
     +"',numA ='"+numA
     +"',numB ='"+numB
     +"',numC ='"+numC
     +"',numD ='"+numD
...
exports.sql_list = function (req, res) {
    ...
    var sql1 = "select * from ordergh order by date desc limit "+(page-1)*limit+","+limit;
    var sql5 = "select count(*) as count from ordergh";
    mysql.query(sql1,function (err, rows1) {
        ...
        mysql.query(sql5,function (err1, rows5) {
            ...
            mysql.query(sql6,function (err1, rows6) {
              if(err1){console.log(err1);return false;}
              //console.log(rows6);
              res.render('cms/dir_gh', {rows6:rows6,url:req.url,address:rows1,page:page,total:total,totalpage:totalpage,isFirstPage:isFirstPage,isLastPage:isLastPage,info:_info});
            });
        });
    });
};