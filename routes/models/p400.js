...
exports.sqldo = function (req, res) {
    ...
    if(_sql == "checkUser"){checkUser(req,res);}
    else if(_sql == "createDoc"){createbooking(req,res);}
    else if(_sql == "createDocFront"){createbookingfront(req,res);}
    else if(_sql == "createDocWeixin"){createbookingweixin(req,res);}
    else if(_sql == "success"){_400success(req,res);}
    else if(_sql == "getpay"){getpay(req,res);}
};
    ...
    mysql.query(sql1 ,function(err1,rows1){
      ...
      mysql.query(sql2 ,function(err2,rows2){
        ...
        mysql.query(sql3 ,function(err3,rows3){
          ...
          res.render('cms/400',{url:req.url,user_s:rows1,address_s:rows2,menu_s:rows3});
        });  
      });
    });
};
...
function createbooking(req,res){
  ...
  if(errs){console.log(errs);return false;}
  ...
  mysql.query(sql1 ,function(err1,rows1){
      ...
      mysql.query(sql32 ,function(err32,rows32){
        if(err32){console.log(err32);return false;}
      });
      //减库存
          ...
          for(var i = 0;i < menu_len;i++){
            if(Number(detail_4[i]!=0)){
                ...
                mysql.query(sql5 ,function(err5,rows5){
                  if(err5){console.log(err5);return false;}
                });
            }
          }
      }else{
...
function checkUser(req,res){
  ...
  mysql.query(sql1 ,function(error,obj){
    ...
    if(!obj[0]){info = "p1@";}
      ...
      if(obj[0].number==0){info = "p2@"+obj[0].name;}
      else{info = "p3@"+obj[0].name;}
    }
    res.send(info);
  });
}