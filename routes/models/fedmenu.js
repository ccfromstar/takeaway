...
function sql_delete(req, res) {
    var id = req.param('id');
    var deleteSql = "delete from fedmenu where id = "+id;
...
function sql_select(req, res) {
    var id = req.param('id');
    var delSql = "select * from fedmenu where id = "+id;
...
exports.sql_list = function (req, res) {
    ...
    var sql1 = "select * from fedmenu order by date desc,id desc limit "+(page-1)*limit+","+limit;
    //var sql1 = "select * from fedmenu order by date desc";
    var sql5 = "select count(*) as count from fedmenu";
    mysql.query(sql1,function (err, rows1) {
        ...
        mysql.query(sql5,function (err1, rows5) {
            ...
            mysql.query(sql6,function (err1, rows6) {
              if(err1){console.log(err1);return false;}
              //console.log(rows6);
              res.render('cms/dir_fedmenu', {rows6:rows6,url:req.url,address:rows1,page:page,total:total,totalpage:totalpage,isFirstPage:isFirstPage,isLastPage:isLastPage,info:_info});
            });
        });
    });
};